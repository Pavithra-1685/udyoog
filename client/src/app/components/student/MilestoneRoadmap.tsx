    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="text-xl font-bold text-[#111111] flex items-center gap-2">
          <Flag className="w-6 h-6 text-[var(--gold-medium)]" />
          Udyoog Roadmap
        </h3>
      </div>
            ...
            return (
              ...
              >
                ...
                <div className={`
                  ...
                  ${isActive ? 'bg-[#111111] shadow-lg shadow-[#111111]/20 rotate-12' : 'bg-white border-2 border-gray-100 text-gray-300'}
                `}>
                  <Icon className={`w-7 h-7 ${isActive ? 'text-[var(--gold-medium)]' : ''}`} />
                  {isActive && (
                    <motion.div 
                      layoutId="check"
                      className="absolute -top-2 -right-2 bg-[var(--gold-gradient)] text-white rounded-full p-1 shadow-md"
                    >
                ...
                <div className={`
                  ...
                  ${isActive ? 'bg-white border-[#111111]/10 shadow-md' : 'bg-gray-50/50 border-gray-100 opacity-60'}
                `}>
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                    <div>
                      <h4 className={`font-bold text-lg ${isActive ? 'text-[#111111]' : 'text-gray-400'}`}>
                        {milestone.title}
                      </h4>
                      <p className="text-sm text-gray-500">{milestone.description}</p>
                    </div>
                    <div className="text-right">
                      <span className={`text-xs font-black uppercase tracking-widest ${isActive ? 'text-[var(--gold-medium)]' : 'text-gray-300'}`}>
                        {milestone.target}
                      </span>
                    </div>
                  </div>
                  ...
                  {milestone.isCompleted && (
                    <div className="flex items-center gap-2 text-[#111111] text-xs font-bold uppercase tracking-widest">
