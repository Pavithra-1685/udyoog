          ...
          <section className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
            ...
            <form onSubmit={handleSearch} className="relative">
              <input
                ...
                className="w-full pl-12 pr-4 py-4 rounded-2xl border border-gray-200 focus:ring-2 focus:ring-[var(--gold-medium)] focus:border-transparent outline-none transition-all"
                value={searchQuery}
              ...
              <button 
                type="submit"
                className="absolute right-3 top-1/2 -translate-y-1/2 px-6 py-2 bg-[#111111] text-white rounded-xl hover:bg-[#111111] transition-colors"
              >
                Search
              </button>
            </form>
            ...
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-[#111111] flex items-center gap-2">
                ...
                {filterMode !== 'all' && (
                  <span
                    className={
                      'text-[10px] px-2.5 py-1 rounded-full font-black uppercase tracking-wider border ml-2 ' +
                      (filterMode === 'skilled'
                  ...
                  {filteredStudents.map((student) => (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 bg-white rounded-xl flex items-center justify-center text-[#111111] font-bold border border-gray-100 shadow-sm">
                          {student.full_name?.charAt(0)}
                        </div>
                        <div>
                          <div className="font-bold text-[#111111] group-hover:text-[var(--gold-medium)] transition-colors">{student.full_name}</div>
                          <div className="text-xs text-gray-500 font-mono">
                            ROLL NO: {student.sif_no || student.registration_no} • {student.home_location || 'Not Specified'}
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center gap-6">
                        <div className="flex flex-col items-end">
                           <div className="text-sm font-bold text-[#111111]">{student.cgpa || '0.0'} CGPA</div>
                           <div className="hidden sm:flex gap-1 mt-1">
                            {student.skills?.slice(0, 3).map((skill: any) => (
                              <span key={typeof skill === 'string' ? skill : skill.name} className="px-2 py-0.5 bg-gray-50 text-[#111111] text-[10px] font-bold rounded uppercase">
                                {typeof skill === 'string' ? skill : skill.name}
                              </span>
                            ))}
                            {(!student.skills || student.skills.length === 0) && (
                              <span className="px-2 py-0.5 bg-[#f4f1e6] text-orange-500 text-[10px] font-bold rounded uppercase">
                                No Skills
                              </span>
                            )}
                          </div>
                        </div>
                        <ChevronRight className="w-5 h-5 text-gray-300 group-hover:text-[var(--gold-medium)] transition-all" />
                      </div>
                    </motion.div>
                  ))}
                </div>
...
function StatCard({ icon: Icon, label, value, color, isActive, onClick }: any) {
  const colors: any = {
    blue: 'bg-gray-50 text-[#111111]',
    green: 'bg-gray-50 text-[#111111]',
    orange: 'bg-[#f4f1e6] text-[var(--gold-medium)]'
  };

  return (
    <div 
      className={`bg-white p-8 rounded-3xl border shadow-sm cursor-pointer transition-all hover:shadow-md ${
        isActive ? 'border-[var(--gold-medium)] ring-2 ring-[var(--gold-medium)]/10' : 'border-gray-100 hover:border-gray-200'
      }`}
      onClick={onClick}
    >
      ...
      <div className="text-3xl font-black text-[#111111]">{value}</div>
